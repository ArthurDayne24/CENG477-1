...
parser::Vec3f parser::calculateCrossProduct(const parser::Vec3f &v1, const parser::Vec3f &v2) {
    parser::Vec3f result;
...
void normalAddition(const parser::Vec3f &normal, parser::Vec3f &vertexData) {
    vertexData.count++;
    vertexData.normalX += normal.x;
    vertexData.normalY += normal.y;
    vertexData.normalZ += normal.z;
}

void parser::doAverageNormal(parser::Vec3f &vertexData) {
    vertexData.normalX /= vertexData.count;
    vertexData.normalY /= vertexData.count;
    vertexData.normalZ /= vertexData.count;

}

parser::Vec3f doOperationOver(const parser::Vec3f v1, const parser::Vec3f v2) {
    parser::Vec3f result;
...
parser::Vec3f parser::doUnitVector(const parser::Vec3f &v) {
    parser::Vec3f result;
    float magnitude = 0;
    magnitude += v.x * v.x;
    magnitude += v.y * v.y;
...
void parser::Scene::loadFromXml(const std::string& filepath)
{
    ...
    while (element)
    {
        ...
        while (!(stream >> face.v0_id).eof())
        {
            ...
            parser::Vec3f v0 = vertex_data[face.v0_id-1];
            parser::Vec3f v1 = vertex_data[face.v1_id-1];
            parser::Vec3f v2 = vertex_data[face.v2_id-1];
            parser::Vec3f normal = parser::calculateCrossProduct((v1-v0),(v2-v0));
            normal = doUnitVector(normal);
            normalAddition(normal,vertex_data[face.v0_id-1]);
            normalAddition(normal,vertex_data[face.v1_id-1]);
            normalAddition(normal,vertex_data[face.v2_id-1]);
    ...
    for(int i = 0; i < vertex_data.size();i++){
        doAverageNormal(vertex_data[i]);
        parser::Vec3f normal;
        normal.x = vertex_data[i].normalX;
        normal.y = vertex_data[i].normalY;
        normal.z = vertex_data[i].normalZ;

        normal = doUnitVector(normal);

        vertex_data[i].normalX = normal.x;
        vertex_data[i].normalY = normal.y;
        vertex_data[i].normalZ = normal.z;
    }
